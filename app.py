...
st.markdown(
    ...
    /* SIDEBAR MODERN (DIJADIKAN FILTER PANEL SEPERTI SUPERSET) */
...
def iku_card(code: str, title: str, value: float | None, formula: str, note: str, status: str = "Tersedia") -> None:
    ...
    if status == "Tersedia":
        if value is not None and (target is None or value >= target):
            badge, label = "badge-success", "Target tercapai"
        else:
            badge, label = "badge-warning", "Belum mencapai target"
    elif status in {"Sebagian", "Proxy"}:
        badge, label = "badge-warning", status
    else:
        badge, label = "badge-danger", "Data belum tersedia"
...
def empty_dashboard(title: str, year: int, icon: str) -> None:
    st.markdown(
        ...
        <div class="empty-card">
            ...
            <p>Halaman ini sudah memiliki navigasi dan filter. Data, KPI, serta visualisasi akan ditambahkan setelah sumber data resmi tersedia.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def create_excel_export(title: str, metrics: dict[str, Any], data: pd.DataFrame) -> bytes:
    output = BytesIO()
    metric_df = pd.DataFrame({"Indikator": list(metrics.keys()), "Nilai": list(metrics.values())})
...
def create_pdf_export(title: str, subtitle: str, metrics: dict[str, Any], data: pd.DataFrame) -> bytes:
    ...
    document = SimpleDocTemplate(
        output, pagesize=landscape(A4), rightMargin=12 * mm, leftMargin=12 * mm, topMargin=12 * mm, bottomMargin=12 * mm, title=title,
    )
    ...
    metric_rows = [[Paragraph("Indikator", header_style), Paragraph("Nilai", header_style)]]
    for label, value in metrics.items():
        metric_rows.append([Paragraph(_safe_export_value(label), cell_style), Paragraph(_safe_export_value(value), cell_style)])
    ...
    story.extend([metric_table, Spacer(1, 8 * mm), Paragraph("Ringkasan Data", styles["Heading2"]), Spacer(1, 3 * mm)])

    export_data = data.copy()
    if export_data.empty:
        story.append(Paragraph("Tidak ada data sesuai filter yang dipilih.", styles["Normal"]))
    else:
        max_columns = 10
        if len(export_data.columns) > max_columns:
            export_data = export_data.iloc[:, :max_columns]
        headers = [Paragraph(_safe_export_value(column), header_style) for column in export_data.columns]
        table_rows = [headers]
        for row in export_data.itertuples(index=False, name=None):
            table_rows.append([Paragraph(_safe_export_value(value), cell_style) for value in row])
...
def export_summary_panel(title: str, subtitle: str, metrics: dict[str, Any], data: pd.DataFrame, filename_prefix: str, key_prefix: str) -> None:
    section("Ringkasan Data dan Export", "Ringkasan ini mengikuti seluruh filter yang sedang aktif. Gunakan tombol untuk mengunduh laporan PDF atau data Excel.")
    metric_table = pd.DataFrame({"Indikator": list(metrics.keys()), "Nilai": list(metrics.values())})
    st.dataframe(metric_table, use_container_width=True, hide_index=True)
    export_columns = st.columns([1, 1, 3])
    with export_columns[0]:
        excel_bytes = create_excel_export(title, metrics, data)
        st.download_button(label="⬇ Export Excel", data=excel_bytes, file_name=f"{filename_prefix}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"{key_prefix}_excel", use_container_width=True)
    with export_columns[1]:
        pdf_bytes = create_pdf_export(title, subtitle, metrics, data)
        st.download_button(label="⬇ Export PDF", data=pdf_bytes, file_name=f"{filename_prefix}.pdf", mime="application/pdf", key=f"{key_prefix}_pdf", use_container_width=True)
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for frame in [mahasiswa, difabel, yudisium]:
        if "nim" in frame.columns:
            frame["nim"] = frame["nim"].astype(str).str.replace(r"\.0$", "", regex=True).str.strip()
    ...
    for column in ["peminat", "lulus_seleksi", "daftar_ulang"]:
        pmb[column] = pd.to_numeric(pmb[column], errors="coerce").fillna(0)
    pmb["jenjang_normal"] = pmb["jenjang"].fillna("").apply(normalize_level)
    for column in ["fakultas", "jurusan", "jenis_seleksi"]:
        pmb[column] = pmb[column].fillna("Tidak diketahui").astype(str).str.strip()

    for column in ["tanggal_sk_akreditasi", "masa_mulai_sk_akreditasi", "masa_sk_akhir_akreditasi"]:
        akreditasi[column] = pd.to_datetime(akreditasi[column], errors="coerce")
    for column in ["jenjang", "fakultas", "prodi", "no_sk_akreditasi", "peringkat"]:
        akreditasi[column] = akreditasi[column].fillna("Tidak diketahui").astype(str).str.strip()
...
MENU_OPTIONS = [
    "Dashboard Landing Page", "Dashboard Akademik", "Dashboard SDM",
    "Dashboard Keuangan", "Dashboard Aset", "Dashboard Riset",
    "Dashboard Alumni", "Prestasi Mahasiswa", "Paperless (PLO)", "Dashboard IKU"
]

landing_years = sorted(
    set(df["tahun_angkatan"].dropna().astype(int).tolist()) |
    set(df_yudisium["tahun_lulus"].dropna().astype(int).tolist()) |
    set(df_pmb["tahun"].dropna().astype(int).tolist()),
    reverse=True
)

academic_year_options = [
    "2026/2027 Genap", "2026/2027 Ganjil", "2025/2026 Genap", "2025/2026 Ganjil",
    "2024/2025 Genap", "2024/2025 Ganjil",
]
...
# ============================================================
# SIDEBAR (FILTER PANEL SEPERTI SUPERSET)
# ============================================================
...
with st.sidebar:
    ...
    if selected_menu == "Dashboard Landing Page":
        st.markdown('<div class="filter-heading">GLOBAL FILTER</div>', unsafe_allow_html=True)
        landing_year = st.selectbox("Tahun Analisis", landing_years, index=0, key="landing_year")

    elif selected_menu == "Dashboard Akademik":
        st.markdown('<div class="filter-heading">WAKTU & PERIODE</div>', unsafe_allow_html=True)
        academic_period = st.selectbox("Tahun Akademik", academic_year_options, index=3, key="academic_period")
        selected_cohorts = st.multiselect("Tahun Angkatan", cohort_options, key="academic_cohorts")
        
        st.markdown('<div class="filter-heading">DEMOGRAFI AKADEMIK</div>', unsafe_allow_html=True)
        selected_levels = st.multiselect("Jenjang", level_options, key="academic_levels")
        selected_faculties = st.multiselect("Fakultas", faculty_options, key="academic_faculties")
        ...
        selected_programs = st.multiselect("Program Studi", program_options, key="academic_programs")
        
        st.markdown('<div class="filter-heading">LAINNYA</div>', unsafe_allow_html=True)
        selected_paths = st.multiselect("Jalur Masuk", path_options, key="academic_paths")
        selected_provinces = st.multiselect("Propinsi Asal", province_options, key="academic_provinces")
        selected_statuses = st.multiselect("Status Mahasiswa", status_options, key="academic_statuses")

        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("↻ Reset Filter", key="reset_filter_akademik"):
            for state_key in [
                "academic_period", "academic_cohorts", "academic_levels", "academic_faculties", 
                "academic_programs", "academic_paths", "academic_provinces", "academic_statuses"
            ]:
        ...
        st.markdown('<div class="filter-heading">GLOBAL FILTER</div>', unsafe_allow_html=True)
        iku_year = st.selectbox("Tahun Capaian IKU", available_iku_years, index=0, key="iku_year")

    else:
        st.markdown('<div class="filter-heading">GLOBAL FILTER</div>', unsafe_allow_html=True)
        page_year = st.number_input("Tahun", min_value=2000, max_value=2100, value=2026, step=1, key=f"{selected_menu}_year")
...
if selected_menu == "Dashboard Landing Page":
    hero(
        ...
        "Ringkasan strategis mahasiswa, PMB, kelulusan, dan indikator institusi."
    )

    landing_students = df[df["tahun_angkatan"].eq(landing_year)]
    landing_active = df[df["status_normal"].eq("aktif")]
    landing_graduates = df_yudisium[df_yudisium["tahun_lulus"].eq(landing_year)]

    pmb_years = sorted(df_pmb["tahun"].dropna().astype(int).unique().tolist())
    landing_pmb_year = landing_year if landing_year in pmb_years else max(pmb_years)
    ...
    if landing_pmb_year != landing_year:
        st.warning(f"Data PMB tahun {landing_year} belum tersedia. Ringkasan PMB memakai tahun terbaru {landing_pmb_year}.")

    section("Ringkasan Eksekutif", "Beberapa indikator sementara menggunakan nilai sampel dan diberi label jelas.")

    executive_cols = st.columns(6)
    executive_values = [
        ("👥", "Mahasiswa aktif", format_number(len(landing_active)), "Data aktual semua angkatan"),
        ("🆕", f"Mahasiswa angkatan {landing_year}", format_number(len(landing_students)), "Data aktual"),
        ("📝", "Peminat PMB", format_number(total_peminat), f"Data aktual PMB {landing_pmb_year}"),
        ("🎓", "Lulusan", format_number(landing_graduates["nim"].nunique()), f"Data aktual yudisium {landing_year}"),
    ...
    with chart_a:
        ...
        figure = px.bar(
            faculty_summary, x="Mahasiswa", y="fakultas", orientation="h", text="Mahasiswa",
            title="Mahasiswa Aktif per Fakultas", color_discrete_sequence=[C["primary"]],
        )
    ...
    with chart_b:
        ...
        figure = px.pie(
            level_summary, names="Jenjang", values="Mahasiswa", hole=0.58, title="Komposisi Jenjang Mahasiswa Aktif",
            color_discrete_sequence=[C["primary"], C["yellow"], C["blue"], C["orange"], C["purple"]],
        )
...
# ============================================================
    ...
    hero(
        "Dashboard Akademik",
        "Profil akademik terintegrasi. Filter di panel kiri mempengaruhi seluruh visualisasi di halaman ini."
    )
    ...
    academic_tabs = st.tabs(
        ["Program Studi", "Mahasiswa", "PMB", "Kelulusan", "Kebutuhan Khusus", "Daerah Tertinggal"]
    )

    with academic_tabs[0]:
        section(
            "Akreditasi Program Studi",
            "Status, peringkat, nomor SK, masa berlaku, dan program studi yang perlu segera memperbarui akreditasi."
        )
        ...
        accreditation_values = [
            ...
            ("⏳", "Berakhir ≤ 1 tahun", segera_berakhir, "Perlu perhatian dan tindak lanjut"),
            ("⚠️", "Kedaluwarsa", akreditasi_kedaluwarsa, "Masa berlaku telah berakhir"),
        ]

        for column, item in zip(accreditation_kpis, accreditation_values):
            with column:
                kpi(item[0], item[1], format_number(item[2]), item[3])
        ...
        with accreditation_left:
            ...
            figure = px.pie(
                rank_summary, names="Peringkat", values="Program Studi", hole=0.56, title="Komposisi Peringkat Akreditasi",
                color_discrete_sequence=[C["primary"], C["yellow"], C["blue"], C["orange"], C["purple"], C["gray"]],
            )
        ...
        with accreditation_right:
            ...
            figure = px.bar(
                faculty_accreditation, x="fakultas", y="Program Studi", color="peringkat", barmode="stack",
        ...
        section(
            ...
            "Tabel terpadu akreditasi program studi, termasuk peringkat, nomor SK, masa berlaku, status aktif, dan prioritas perpanjangan."
        )

        accreditation_table = (
            akreditasi_filter[
                ["jenjang_normal", "fakultas", "prodi", "peringkat", "no_sk_akreditasi", "tanggal_sk_akreditasi", "masa_mulai_sk_akreditasi", "masa_sk_akhir_akreditasi", "status_akreditasi", "masa_berakhir", "sisa_hari"]
            ]
        ...
        st.dataframe(
            accreditation_table, use_container_width=True, hide_index=True,
            column_config={
        ...
        export_summary_panel(
            ...
            subtitle=f"Dashboard Akademik - {academic_period}",
            metrics={
                "Periode Akademik": academic_period, "Jumlah Program Studi": total_prodi_akreditasi,
                "Akreditasi Aktif": akreditasi_aktif, "Peringkat Unggul": jumlah_unggul,
                "Berakhir Dalam 1 Tahun": segera_berakhir, "Akreditasi Kedaluwarsa": akreditasi_kedaluwarsa,
            },
            data=accreditation_table, filename_prefix="ringkasan_akreditasi_program_studi", key_prefix="export_program_studi",
        )

    with academic_tabs[1]:
        section(
            "Mahasiswa",
            "Ringkasan populasi mahasiswa, program studi, jenis kelamin, dan persebaran asal propinsi sesuai filter akademik."
        )
        ...
        section("Sebaran Mahasiswa per Program Studi", "Dua puluh program studi dengan jumlah mahasiswa terbanyak berdasarkan filter yang dipilih.")

        prodi_base = academic_df.groupby(["fakultas", "jurusan", "jenjang_normal"], dropna=False).agg(Total_Mahasiswa=("nim", "nunique")).reset_index()
        prodi_status = academic_df.pivot_table(index=["fakultas", "jurusan", "jenjang_normal"], columns="status_kategori", values="nim", aggfunc=pd.Series.nunique, fill_value=0).reset_index()

        status_column_order = [s for s in preferred_status_order if s in prodi_status.columns]
        status_column_order.extend(sorted(c for c in prodi_status.columns if c not in {"fakultas", "jurusan", "jenjang_normal"} and c not in status_column_order))

        status_export_names = {"Aktif": "Mahasiswa Aktif", "Lulus": "Lulus", "Cuti": "Cuti", "Tidak Aktif": "Tidak Aktif", "DO/Putus Studi": "DO/Putus Studi", "Mengundurkan Diri": "Mengundurkan Diri", "Meninggal": "Meninggal", "Skorsing": "Skorsing", "Pindah/Transfer": "Pindah/Transfer", "Tidak Diketahui": "Tidak Diketahui"}
        prodi_status = prodi_status.rename(columns={s: status_export_names.get(s, s) for s in status_column_order})
        renamed_status_columns = [status_export_names.get(s, s) for s in status_column_order]

        prodi_summary = prodi_base.merge(prodi_status, on=["fakultas", "jurusan", "jenjang_normal"], how="left").fillna(0).sort_values("Total_Mahasiswa", ascending=False)
        for column in renamed_status_columns:
            prodi_summary[column] = pd.to_numeric(prodi_summary[column], errors="coerce").fillna(0).astype(int)

        chart_df = prodi_summary.nlargest(20, "Total_Mahasiswa").sort_values("Total_Mahasiswa")
        figure = px.bar(
            chart_df, x="Total_Mahasiswa", y="jurusan", orientation="h", text="Total_Mahasiswa",
            title="20 Program Studi dengan Mahasiswa Terbanyak", color_discrete_sequence=[C["primary"]],
            hover_data={"fakultas": True, "jenjang_normal": True, "Total_Mahasiswa": True},
        )
        ...
        with st.expander("Lihat ringkasan mahasiswa per program studi"):
            st.dataframe(
                prodi_summary, use_container_width=True, hide_index=True,
                column_config={"fakultas": "Fakultas", "jurusan": "Program Studi", "jenjang_normal": "Jenjang", "Total_Mahasiswa": "Total Mahasiswa", **{c: st.column_config.NumberColumn(c, format="%d") for c in renamed_status_columns}},
            )

        section("Profil dan Persebaran Mahasiswa", "Komposisi jenis kelamin dan peta sebaran asal propinsi mahasiswa.")

        left, right = st.columns([0.75, 1.45])
        with left:
            gender_df = academic_df["gender_normal"].value_counts().rename_axis("Jenis Kelamin").reset_index(name="Mahasiswa")
            figure = px.pie(
                gender_df, names="Jenis Kelamin", values="Mahasiswa", hole=0.58, title="Komposisi Jenis Kelamin",
                color_discrete_sequence=[C["primary"], C["orange"], C["gray"]],
            )
        ...
        with right:
            ...
            if mapped_provinces.empty:
                st.info("Nama propinsi pada data belum dapat dicocokkan dengan koordinat peta.")
            else:
                figure = px.scatter_geo(
                    mapped_provinces, lat="Latitude", lon="Longitude", size="Mahasiswa", color="Mahasiswa", hover_name="Propinsi",
                    hover_data={"Mahasiswa": ":,", "Latitude": False, "Longitude": False}, size_max=42, title="Peta Sebaran Asal Propinsi Mahasiswa", color_continuous_scale="Teal",
                )
                ...
                unmapped_count = int(province_map_df["Latitude"].isna().sum())
                if unmapped_count:
                    with st.expander("Lihat nama propinsi yang belum terpetakan"):
                        st.dataframe(province_map_df[province_map_df["Latitude"].isna()][["Propinsi", "Mahasiswa"]], use_container_width=True, hide_index=True)

        mahasiswa_export_data = prodi_summary.copy()
        mahasiswa_export_data["Periode Akademik"] = academic_period
        export_summary_panel(
            title="Ringkasan Data Mahasiswa", subtitle=f"Dashboard Akademik - {academic_period}",
            metrics={
                "Periode Akademik": academic_period, "Total Mahasiswa": len(academic_df),
                **{s: int(status_counts.get(s, 0)) for s in visible_statuses},
                "Jumlah Fakultas": academic_df["fakultas"].nunique(), "Jumlah Program Studi": academic_df["jurusan"].nunique(), "Jumlah Propinsi Asal": academic_df["propinsi"].nunique(),
            },
            data=mahasiswa_export_data, filename_prefix="ringkasan_mahasiswa", key_prefix="export_mahasiswa",
        )

    with academic_tabs[2]:
        section("Penerimaan Mahasiswa Baru", "Rekap peminat, lulus seleksi, dan daftar ulang.")
        ...
        if academic_pmb_year != academic_start_year:
            st.warning(f"Data PMB tahun {academic_start_year} belum tersedia. Dashboard memakai tahun terbaru {academic_pmb_year}.")
        ...
        for column, item in zip(cols, pmb_values):
            with column:
                kpi(item[0], item[1], item[2], f"PMB {academic_pmb_year}")
        ...
        export_summary_panel(
            title="Ringkasan Penerimaan Mahasiswa Baru", subtitle=f"Dashboard Akademik - PMB {academic_pmb_year}",
            metrics={
                "Tahun PMB": academic_pmb_year, "Jumlah Peminat": format_number(total_peminat), "Lulus Seleksi": format_number(total_lulus),
                "Daftar Ulang": format_number(total_daftar), "Yield Rate": format_percent(percentage(total_daftar, total_lulus)),
            },
            data=pmb_summary_export, filename_prefix="ringkasan_pmb", key_prefix="export_pmb",
        )

    with academic_tabs[3]:
        section("Kelulusan", "Ringkasan yudisium, ketepatan waktu kelulusan, dan capaian IPK lulusan berdasarkan tahun awal periode akademik.")
        ...
        if academic_yud.empty:
            ...
            section("Kelulusan per Fakultas", "Jumlah lulusan dan persentase kelulusan tepat waktu berdasarkan fakultas.")
            ...
            with st.expander("Lihat detail data yudisium"):
                ...
                st.dataframe(
                    academic_yud[yudisium_columns], use_container_width=True, hide_index=True,
        ...
        graduation_export_data = academic_yud[[c for c in ["nim", "nama", "tanggal_lulus", "tahun_lulus", "jenjang_normal", "fakultas", "prodi", "semester", "ipk", "tepat_waktu"] if c in academic_yud.columns]].copy()
        export_summary_panel(
            title="Ringkasan Data Kelulusan", subtitle=f"Dashboard Akademik - Yudisium {academic_start_year}",
            metrics={
                "Tahun Yudisium": academic_start_year, "Jumlah Lulusan": graduate_count, "Lulus Tepat Waktu": on_time_count,
                "Tepat Waktu dan IPK >= 3,25": on_time_ipk_count, "Persentase Kelulusan Tepat Waktu": format_percent(on_time_percentage),
                "Rata-rata IPK": (f"{academic_yud['ipk'].mean():.2f}" if not academic_yud.empty and pd.notna(academic_yud["ipk"].mean()) else "-"),
            },
            data=graduation_export_data, filename_prefix="ringkasan_kelulusan", key_prefix="export_kelulusan",
        )

    with academic_tabs[5]:
        section("Mahasiswa Berkebutuhan Khusus", "Profil berdasarkan difabel.xls.")
        ...
        kebutuhan_summary_export = filtered_difabel.groupby(["fakultas", "jurusan", "kebutuhan_khusus"], dropna=False).agg(Jumlah_Mahasiswa=("nim", "nunique"), Mahasiswa_Aktif=("status_normal", lambda values: values.eq("aktif").sum())).reset_index().sort_values("Jumlah_Mahasiswa", ascending=False)
        export_summary_panel(
            title="Ringkasan Mahasiswa Berkebutuhan Khusus", subtitle=f"Dashboard Akademik - {academic_period}",
            metrics={
                "Periode Akademik": academic_period, "Total Mahasiswa Berkebutuhan Khusus": filtered_difabel["nim"].nunique(),
                "Mahasiswa Aktif": filtered_difabel.loc[filtered_difabel["status_normal"].eq("aktif"), "nim"].nunique(),
                "Jumlah Program Studi": filtered_difabel["jurusan"].nunique(), "Jenis Kebutuhan Khusus": filtered_difabel["kebutuhan_khusus"].nunique(),
            },
            data=kebutuhan_summary_export, filename_prefix="ringkasan_mahasiswa_kebutuhan_khusus", key_prefix="export_kebutuhan_khusus",
        )

    with academic_tabs[4]:
        section("Daerah Tertinggal", "Mahasiswa dari kabupaten daerah tertinggal 2025–2029.")
        ...
        disadvantaged_summary = disadvantaged.groupby(["propinsi", "kota_normal"], dropna=False).size().reset_index(name="Mahasiswa").sort_values("Mahasiswa", ascending=False)
        st.dataframe(disadvantaged_summary, use_container_width=True, hide_index=True)

        export_summary_panel(
            title="Ringkasan Mahasiswa Daerah Tertinggal", subtitle=f"Dashboard Akademik - {academic_period}",
            metrics={
                "Periode Akademik": academic_period, "Mahasiswa Daerah Tertinggal": len(disadvantaged),
                "Kabupaten Teridentifikasi": disadvantaged["kota_normal"].nunique(), "Propinsi Teridentifikasi": disadvantaged["propinsi"].nunique(),
                "Persentase dari Mahasiswa Terfilter": format_percent(percentage(len(disadvantaged), len(academic_df))),
            },
            data=disadvantaged_summary, filename_prefix="ringkasan_mahasiswa_daerah_tertinggal", key_prefix="export_daerah_tertinggal",
        )
...
# ============================================================
    ...
    hero("Dashboard SDM", "Kerangka dashboard sumber daya manusia sedang disiapkan.")
    empty_dashboard("Dashboard SDM", page_year, "👩‍🏫")

elif selected_menu == "Dashboard Keuangan":
    hero("Dashboard Keuangan", "Kerangka dashboard keuangan sedang disiapkan.")
    empty_dashboard("Dashboard Keuangan", page_year, "💰")

elif selected_menu == "Dashboard Aset":
    hero("Dashboard Aset", "Kerangka dashboard aset sedang disiapkan.")
    empty_dashboard("Dashboard Aset", page_year, "🏢")

elif selected_menu == "Dashboard Riset":
    hero("Dashboard Riset", "Kerangka dashboard riset dan publikasi sedang disiapkan.")
    empty_dashboard("Dashboard Riset", page_year, "🔬")

elif selected_menu == "Dashboard Alumni":
    hero("Dashboard Alumni", "Kerangka dashboard alumni dan tracer study sedang disiapkan.")
    empty_dashboard("Dashboard Alumni", page_year, "🎓")

elif selected_menu == "Prestasi Mahasiswa":
    hero("Dashboard Prestasi Mahasiswa", "Kerangka dashboard prestasi akademik dan nonakademik mahasiswa sedang disiapkan.")
    empty_dashboard("Dashboard Prestasi Mahasiswa", page_year, "🏆")

elif selected_menu == "Paperless (PLO)":
    hero("Dashboard Paperless (PLO)", "Kerangka dashboard layanan dan proses administrasi paperless sedang disiapkan.")
...
# ============================================================
    ...
    hero("Dashboard Capaian IKU", "Capaian indikator berdasarkan sumber data yang tersedia.")

    iku_tabs = st.tabs(["IKU Akademik", "IKU SDM", "IKU Keuangan", "IKU Aset", "IKU Riset", "IKU Alumni"])

    with iku_tabs[0]:
        new_t = int(df["tahun_angkatan"].eq(iku_year).sum())
        new_previous = int(df["tahun_angkatan"].eq(iku_year - 1).sum())
        iku_01_01 = growth(new_t, new_previous)

        new_students = df[df["tahun_angkatan"].eq(iku_year)]
        disadvantaged_count = int(new_students["asal_daerah_tertinggal"].sum())
        iku_01_07 = percentage(disadvantaged_count, len(new_students))

        dif_t = int(df_difabel["tahun_angkatan"].eq(iku_year).sum())
        dif_previous = int(df_difabel["tahun_angkatan"].eq(iku_year - 1).sum())
        iku_02_01 = growth(dif_t, dif_previous)

        yud_year = df_yudisium[df_yudisium["tahun_lulus"].eq(iku_year)]
        ...
        pmb_years = sorted(df_pmb["tahun"].dropna().astype(int).unique().tolist())
        iku_pmb_year = iku_year if iku_year in pmb_years else max(pmb_years)
        ...
        if iku_pmb_year != iku_year:
            st.warning(f"Data PMB tahun {iku_year} belum tersedia. IKU-35-54 memakai data PMB terbaru {iku_pmb_year}.")
    ...
    with iku_tabs[1]: empty_dashboard("IKU SDM", iku_year, "👩‍🏫")
    with iku_tabs[2]: empty_dashboard("IKU Keuangan", iku_year, "💰")
    with iku_tabs[3]: empty_dashboard("IKU Aset", iku_year, "🏢")
    with iku_tabs[4]: empty_dashboard("IKU Riset", iku_year, "🔬")
    with iku_tabs[5]: empty_dashboard("IKU Alumni", iku_year, "🎓")
