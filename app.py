...
with main_tabs[1]:
    hero("Dashboard Akademik", "Profil akademik terintegrasi. Filter di panel kiri mempengaruhi seluruh visualisasi di tab ini.")
    ...
    # Menggunakan radio buttons horizontal sebagai "Sub-Tabs"
    ...
    # 2.A PROGRAM STUDI
    if academic_section == "Program Studi":
        section("Akreditasi Program Studi", "Status, peringkat, nomor SK, masa berlaku, dan program studi yang perlu segera memperbarui akreditasi.")
        ...
        section("Daftar dan Masa Berlaku Akreditasi Program Studi", "Tabel terpadu akreditasi program studi, termasuk peringkat, nomor SK, masa berlaku, status aktif, dan prioritas perpanjangan.")
        ...
        export_summary_panel(
            title="Ringkasan Akreditasi Program Studi", subtitle=f"Dashboard Akademik - {academic_period}",
            metrics={"Periode Akademik": academic_period, "Jumlah Program Studi": total_prodi_akreditasi, "Akreditasi Aktif": akreditasi_aktif, "Peringkat Unggul": jumlah_unggul, "Berakhir Dalam 1 Tahun": segera_berakhir, "Akreditasi Kedaluwarsa": akreditasi_kedaluwarsa},
    ...
    # 2.B MAHASISWA
        ...
        section("Mahasiswa", "Ringkasan populasi mahasiswa, program studi, jenis kelamin, dan persebaran asal propinsi sesuai filter akademik.")
        ...
        with st.expander("Lihat ringkasan mahasiswa per program studi"):
            st.dataframe(prodi_summary, use_container_width=True, hide_index=True, column_config={"fakultas": "Fakultas", "jurusan": "Program Studi", "jenjang_normal": "Jenjang", "Total_Mahasiswa": "Total Mahasiswa", **{c: st.column_config.NumberColumn(c, format="%d") for c in renamed_status_columns}})
        ...
        export_summary_panel(
            ...
            metrics={"Periode Akademik": academic_period, "Total Mahasiswa": len(academic_df), **{s: int(status_counts.get(s, 0)) for s in visible_statuses}, "Jumlah Fakultas": academic_df["fakultas"].nunique(), "Jumlah Program Studi": academic_df["jurusan"].nunique(), "Jumlah Propinsi Asal": academic_df["propinsi"].nunique()},
            data=mahasiswa_export_data, filename_prefix="ringkasan_mahasiswa", key_prefix="export_mahasiswa",
        )

    # 2.C PMB
    elif academic_section == "PMB":
        ...
        export_summary_panel(
            ...
            metrics={"Tahun PMB": academic_pmb_year, "Jumlah Peminat": format_number(total_peminat), "Lulus Seleksi": format_number(total_lulus), "Daftar Ulang": format_number(total_daftar), "Yield Rate": format_percent(percentage(total_daftar, total_lulus))},
    ...
    # 2.D KELULUSAN
        ...
        if academic_yud.empty:
            ...
            with st.expander("Lihat detail data yudisium"):
                ...
                st.dataframe(academic_yud[yudisium_columns], use_container_width=True, hide_index=True, column_config={"nim": "NIM", "nama": "Nama", "tanggal_lulus": st.column_config.DateColumn("Tanggal Yudisium/Lulus", format="DD/MM/YYYY"), "tahun_lulus": "Tahun Lulus", "jenjang_normal": "Jenjang", "fakultas": "Fakultas", "prodi": "Program Studi", "semester": "Semester", "ipk": st.column_config.NumberColumn("IPK", format="%.2f"), "tepat_waktu": "Tepat Waktu"})
        ...
        export_summary_panel(
            ...
            metrics={"Tahun Yudisium": academic_start_year, "Jumlah Lulusan": graduate_count, "Lulus Tepat Waktu": on_time_count, "Tepat Waktu dan IPK >= 3,25": on_time_ipk_count, "Persentase Kelulusan Tepat Waktu": format_percent(on_time_percentage), "Rata-rata IPK": (f"{academic_yud['ipk'].mean():.2f}" if not academic_yud.empty and pd.notna(academic_yud["ipk"].mean()) else "-")},
            data=graduation_export_data, filename_prefix="ringkasan_kelulusan", key_prefix="export_kelulusan",
        )

    # 2.E KEBUTUHAN KHUSUS (DIPASTIKAN TIDAK TERTUKAR DENGAN DAERAH TERTINGGAL)
        ...
        export_summary_panel(
            ...
            metrics={"Periode Akademik": academic_period, "Total Mahasiswa Berkebutuhan Khusus": filtered_difabel["nim"].nunique(), "Mahasiswa Aktif": filtered_difabel.loc[filtered_difabel["status_normal"].eq("aktif"), "nim"].nunique(), "Jumlah Program Studi": filtered_difabel["jurusan"].nunique(), "Jenis Kebutuhan Khusus": filtered_difabel["kebutuhan_khusus"].nunique()},
            data=kebutuhan_summary_export, filename_prefix="ringkasan_mahasiswa_kebutuhan_khusus", key_prefix="export_kebutuhan_khusus",
        )

    # 2.F DAERAH TERTINGGAL (DIPASTIKAN TIDAK TERTUKAR DENGAN KEBUTUHAN KHUSUS)
        ...
        export_summary_panel(
            ...
            metrics={"Periode Akademik": academic_period, "Mahasiswa Daerah Tertinggal": len(disadvantaged), "Kabupaten Teridentifikasi": disadvantaged["kota_normal"].nunique(), "Propinsi Teridentifikasi": disadvantaged["propinsi"].nunique(), "Persentase dari Mahasiswa Terfilter": format_percent(percentage(len(disadvantaged), len(academic_df)))},
